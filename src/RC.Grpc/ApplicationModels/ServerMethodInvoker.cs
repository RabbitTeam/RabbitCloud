...
namespace Rabbit.Cloud.Grpc.ApplicationModels
{
    public abstract class ServerMethodInvoker : IServerMethodInvoker
    {
        ...
        protected virtual MethodModel ServerMethod { get; }
        protected virtual IServiceProvider Services { get; }

        protected ServerMethodInvoker(MethodModel serverMethod, IServiceProvider services, ILogger logger)
        {
            _logger = logger;
        ...
        protected virtual object GetServiceInstance()
        {
            try
            {
                var serviceType = ServerMethod.ServiceModel.Type;
