...
namespace Rabbit.Cloud.Grpc.ApplicationModels
{
    ...
    public class ServerServiceDefinitionProvider : IServerServiceDefinitionProvider
    {
        ...
        private IServerMethodInvoker GetServerMethodInvoker(MethodModel serviceMethod)
        {
        ...
        public void OnProvidersExecuting(ServerServiceDefinitionProviderContext context)
        {
            ...
            foreach (var serverService in applicationModel.Services)
            {
                foreach (var serverServiceMethod in serverService.Methods)
                {
                    var serviceId = $"/{serverService.ServiceName}/{serverServiceMethod.Name}";

                    var grpcMethod = _methodTable.Get(serviceId);
                    var requestType = serverServiceMethod.RequestCodec.Type;
                    var responseType = serverServiceMethod.ResponseCodec.Type;
        ...
        private static class Cache
        {
            ...
            private static Delegate GetUnaryMethodDelegate(IServerMethodInvoker serverMethodInvoker, MethodModel serverMethod)
            {
                var key = ("Unary", serverMethod.MethodInfo);
                return GetCache(key, () =>
                {
                    var requestType = serverMethod.RequestCodec.Type;
                    var responseType = serverMethod.ResponseCodec.Type;
